...
FORWARD_WEBHOOK_URLS = env_str("WONYODD_FORWARD_WEBHOOK_URLS", "")
FORWARD_WEBHOOK_TIMEOUT_SEC = int(env_float("WONYODD_FORWARD_WEBHOOK_TIMEOUT_SEC", 8))
...
# READY auto-notify (when recommend status becomes READY)
READY_NOTIFY_ENABLED = env_bool("WONYODD_READY_NOTIFY_ENABLED", False)
READY_NOTIFY_TFS = env_str("WONYODD_READY_NOTIFY_TFS", "30m,60m,180m")  # comma-separated
READY_NOTIFY_SIDE = env_str("WONYODD_READY_NOTIFY_SIDE", "both")  # auto|long|short|both
READY_NOTIFY_ONLY_BAR_CLOSE = env_bool("WONYODD_READY_NOTIFY_ONLY_BAR_CLOSE", True)
READY_NOTIFY_COOLDOWN_SEC = int(env_float("WONYODD_READY_NOTIFY_COOLDOWN_SEC", 0))
...
COUPANG_ACCESS_KEY = env_str("COUPANG_ACCESS_KEY", "")
COUPANG_SECRET_KEY = env_str("COUPANG_SECRET_KEY", "")
COUPANG_SUB_ID = env_str("COUPANG_SUB_ID", "cp-banner")
COUPANG_BANNER_DEFAULT_CATEGORY = env_str("COUPANG_BANNER_DEFAULT_CATEGORY", "daily")
COUPANG_API_INFO_FILE = env_str("COUPANG_API_INFO_FILE", "/home/ubuntu/쿠팡파트너스api정보.txt")
COUPANG_LINKS_FILE = env_str("COUPANG_LINKS_FILE", "/home/ubuntu/쿠팡광고링크.txt")
